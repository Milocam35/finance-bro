...
import { GraduationCap, ArrowUpDown, AlertCircle, RefreshCw, SearchX, TrendingDown, ShieldCheck, BarChart3, Building2 } from "lucide-react";
import { EducationCard } from "./EducationCard";
import { EducationFilters, EducationFilterState } from "./EducationFilters";
import { useProductosEducativos } from "./useProductosEducativos";
...
export function EducationComparison() {
  const { data: productos, isLoading, isError, error, refetch } = useProductosEducativos();
  ...
  const filteredProductos = (productos ?? []).filter((producto) => {
    ...
    if (filters.educationType !== "all") {
      const text = `${producto.descripcion ?? ""} ${producto.id_unico_scraping ?? ""}`.toLowerCase();
      if (filters.educationType === "posgrado") {
        const isPosgrado = /posgrado|postgrado|maestr[ií]a|doctorado|especializaci[oó]n|mba/.test(text);
        if (!isPosgrado) return false;
      } else if (filters.educationType === "pregrado") {
        const isPosgrado = /posgrado|postgrado|maestr[ií]a|doctorado|especializaci[oó]n|mba/.test(text);
        if (isPosgrado) return false;
      }
    }
  ...
  return (
    <>
      ...
      <section className="relative pt-24 pb-20 lg:pt-28 lg:pb-28 overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center py-12 lg:py-16 max-w-4xl mx-auto">
            ...
            >
              Compara tasas de crédito educativo de los principales bancos colombianos.
              <br className="hidden md:block" />
              Encuentra la mejor opción para financiar tus estudios.
            </motion.p>
      ...
      <section className="relative pt-8 pb-16 lg:pb-24">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="mb-10">
            <EducationFilters onFilterChange={setFilters} />
          </div>
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-muted-foreground mb-6 max-w-md">
                {error instanceof Error
                  ? error.message
                  : "No se pudieron cargar los créditos educativos. Verifica que el servidor esté funcionando."}
              </p>
              ...
              {sortedProductos.length === 0 && (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="text-muted-foreground max-w-md">
                    No hay créditos educativos que coincidan con los filtros seleccionados.
