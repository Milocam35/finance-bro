...
import { Car, ArrowUpDown, AlertCircle, RefreshCw, SearchX, TrendingDown, ShieldCheck, BarChart3, Building2 } from "lucide-react";
import { useQuery } from "@tanstack/react-query";
import { VehicleCard } from "./VehicleCard";
import { VehicleFilters, VehicleFilterState } from "./VehicleFilters";
import { useProductosVehiculo } from "./useProductosVehiculo";
...
export function VehicleComparison() {
  const { data: productos, isLoading, isError, error, refetch } = useProductosVehiculo();
  ...
  const filteredProductos = (productos ?? []).filter((producto) => {
    ...
    if (filters.vehicleType !== "all") {
      ...
      if (filters.vehicleType === "moto") {
        const isMoto = /moto|motocicleta|ciclomotor/.test(text);
        if (!isMoto) return false;
      } else if (filters.vehicleType === "auto") {
        const isMoto = /moto|motocicleta|ciclomotor/.test(text);
        if (isMoto) return false;
      }
    }
      ...
      <section className="relative pt-24 pb-20 lg:pt-28 lg:pb-28 overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center py-12 lg:py-16 max-w-4xl mx-auto">
            ...
            >
              Compara tasas de crédito vehicular de los principales bancos colombianos.
              <br className="hidden md:block" />
              Encuentra la mejor opción para tu carro o moto.
            </motion.p>
            ...
            >
              {[
                ...
                { icon: Building2, value: "15+", label: "Entidades" },
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-muted-foreground mb-6 max-w-md">
                {error instanceof Error
                  ? error.message
                  : "No se pudieron cargar los créditos de vehículo. Verifica que el servidor esté funcionando."}
              </p>
              ...
              {sortedProductos.length === 0 && (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="text-muted-foreground max-w-md">
                    No hay créditos de vehículo que coincidan con los filtros seleccionados.
