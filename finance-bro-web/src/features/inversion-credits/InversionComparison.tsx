...
export function InversionComparison() {
  ...
  const filteredProductos = (productos ?? []).filter((producto) => {
    if (filters.term && producto.monto?.plazo_maximo_meses) {
      ...
      if (plazoMeses > producto.monto.plazo_maximo_meses) return false;
    }
    return true;
  });
    ...
    <>
      ...
      <section className="relative pt-24 pb-20 lg:pt-28 lg:pb-28 overflow-hidden">
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div className="absolute -bottom-48 -left-24 w-[600px] h-[600px] rounded-full opacity-[0.05]"
        ...
        {floatingParticles.map((p, i) => (
          <motion.div key={i} className="absolute rounded-full"
            style={{ width: p.size, height: p.size, left: p.x, top: p.y, backgroundColor: `hsl(var(--primary-foreground) / 0.25)` }}
          ...
          <div className="text-center py-12 lg:py-16 max-w-4xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 16, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.5 }}>
              <span className="inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full text-sm font-medium mb-8 border"
                style={{ backgroundColor: `hsl(var(--primary-foreground) / 0.08)`, borderColor: `hsl(var(--primary-foreground) / 0.15)`, color: `hsl(var(--primary-foreground) / 0.9)`, backdropFilter: 'blur(8px)' }}>
                <Banknote className="w-4 h-4" />
                Libre Inversión
                <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ backgroundColor: `hsl(var(--accent))` }} />
              </span>
            </motion.div>
            ...
            >
              ...
              <span className="relative inline-block" style={{ color: `hsl(var(--accent))` }}>
                Libre Inversión
                <motion.div
                  initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.8, delay: 0.7, ease: [0.22, 1, 0.36, 1] }}
            ...
            >
              ...
              Úsalo para lo que necesites: viajes, remodelaciones, consolidar deudas y más.
            </motion.p>
            ...
            >
              {[
                ...
                <motion.div key={stat.label}
                  initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.5 + index * 0.08 }}
                  className="flex items-center gap-3 px-5 py-3 rounded-xl border"
                  style={{ backgroundColor: `hsl(var(--primary-foreground) / 0.04)`, borderColor: `hsl(var(--primary-foreground) / 0.08)`, backdropFilter: 'blur(4px)' }}
                >
                  <stat.icon className="w-4 h-4 shrink-0" style={{ color: `hsl(var(--accent) / 0.8)` }} />
                  <div className="text-left">
                    <p className="text-sm font-bold leading-tight" style={{ color: `hsl(var(--primary-foreground) / 0.95)` }}>{stat.value}</p>
                    <p className="text-[11px] leading-tight" style={{ color: `hsl(var(--primary-foreground) / 0.45)` }}>{stat.label}</p>
                  </div>
                </motion.div>
              ))}
            </motion.div>
        ...
        <div className="container mx-auto px-4 relative z-10">
              ...
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="bg-card rounded-2xl border border-border p-6 animate-pulse">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div className="flex-1"><div className="h-5 bg-muted rounded w-32 mb-2" /><div className="h-3 bg-muted rounded w-24" /></div>
                  </div>
                  <div className="flex gap-2 mb-4"><div className="h-6 bg-muted rounded w-16" /><div className="h-6 bg-muted rounded w-20" /></div>
                  <div className="h-20 bg-muted rounded mb-4" />
                  <div className="space-y-3 py-4"><div className="h-4 bg-muted rounded w-full" /><div className="h-4 bg-muted rounded w-3/4" /></div>
                  <div className="flex gap-3 pt-4"><div className="h-10 bg-muted rounded flex-1" /><div className="h-10 bg-muted rounded flex-1" /></div>
                </div>
              ))}
            </div>
          )}
          ...
          {isError && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <h3 className="text-xl font-semibold text-foreground mb-2">Error al cargar los productos</h3>
              <p className="text-muted-foreground mb-6 max-w-md">
                {error instanceof Error ? error.message : "No se pudieron cargar los créditos de libre inversión. Verifica que el servidor esté funcionando."}
              </p>
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  ...
                  Ordenado por: <span className="font-medium text-foreground">
                    {filters.sortBy === "rate" ? "Menor tasa" : "Menor mensualidad"}
                  </span>
                </div>
              </div>
